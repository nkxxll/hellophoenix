defmodule HellophoenixWeb.ItemLive.Form do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:item, item)
    |> assign(:form, to_form(Search.change_item(socket.assigns.current_scope, item)))
  end
  ...
  def handle_event("validate", %{"item" => item_params}, socket) do
    changeset = Search.change_item(socket.assigns.current_scope, socket.assigns.item, item_params)
  ...
  defp save_item(socket, :edit, item_params) do
    case Search.update_item(socket.assigns.current_scope, socket.assigns.item, item_params) do
      {:ok, item} ->
        {:noreply,
         socket
  ...
  defp save_item(socket, :new, item_params) do
    case Search.create_item(socket.assigns.current_scope, item_params) do
      {:ok, item} ->
        {:noreply,
         socket
